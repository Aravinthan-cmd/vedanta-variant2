...
const Table = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setTime(getUpdateSensorData6[0].TIME)
    };
  ...
  useEffect(() => {
  setTimeTable(formatTime(time));
  setDateTable(formatDate(time));
  },[time]);
  ...
  return (
    ...
    {busbar1.length !== 0 ? (
      ...
      <div className="header">
        ...
        <span className="date">
          <h1>{`Date and Time: ${dateTable} ${timeTable}`}</h1>
        </span>
      </div>
